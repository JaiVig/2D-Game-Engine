...
Level::Level(const std::string& fileName) {
	...
	// Error checking
	if (file.fail()) {
		Crusty::fatalError("Failed to open " + fileName);
	}
		...
		for (int x = 0; x < _levelData[y].size(); x++) {
			...
			case 'R':
				_spriteBatch.draw(destRect,
					uvRect,
					Crusty::ResourceManager::getTexture("Textures/red_bricks.png").id,
					0.0f,
					whiteColor);
				break;
			case 'G':
				_spriteBatch.draw(destRect,
					uvRect,
					Crusty::ResourceManager::getTexture("Textures/glass.png").id,
					0.0f,
					whiteColor);
				break;
			case 'L':
				_spriteBatch.draw(destRect,
					uvRect,
					Crusty::ResourceManager::getTexture("Textures/light_bricks.png").id,
					0.0f,
					whiteColor);
				break;
			case '@':
